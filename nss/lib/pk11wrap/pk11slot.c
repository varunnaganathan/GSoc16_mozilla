...
SECStatus
PK11_PrintURIFromToken(PK11SlotInfo *slot) {
    //This is the libp11 uri struct required for the format function call
    P11KitUri *uri = p11_kit_uri_new();
    CK_TOKEN_INFO *token = p11_kit_uri_get_token_info(uri);
    //The URI string
    char *string = NULL;
    //This fills the token info into the CK_TOKEN_INFO struct passed
    SECStatus status = PK11_GetTokenInfo(slot, token);
    if (!status) {
        //Raise Error
        return SECFailure;
    }
    //The format function formats the uri in string form and stores into the string parameter passed
    int uristatus = p11_kit_uri_format(uri, P11_KIT_URI_FOR_TOKEN, &string);
    // if uri was successfully formatted
    if(uristatus) {
        printf("%s\n", string);
        return SECSuccess;
    }
